...
common_ids <- intersect(clean_sample_names(rownames(k2_full)),
                        clean_sample_names(rownames(ku_full)))
k2_full <- k2_full[clean_sample_names(rownames(k2_full)) %in% common_ids, ]
ku_full <- ku_full[clean_sample_names(rownames(ku_full)) %in% common_ids, ]
...
# rimozione campioni con somma 0 in almeno un tool
...
# Normalize each sample to relative abundance before Bray-Curtis: standard
# practice in community ecology, removes the effect of different total
# classified reads between samples / between tools.
...
# il segno degli assi PCoA è arbitrario: se PC1 è anticorrelato, lo flippiamo
...
# La media (0.31) nasconde variabilita': poche famiglie abbondanti hanno correlazioni
# molto alte, molte famiglie rare hanno correlazioni basse o NA.
par(mar=c(5, 5, 4, 2))
rhos_clean <- na.omit(rhos)

# breaks dinamici sul range reale dei dati
